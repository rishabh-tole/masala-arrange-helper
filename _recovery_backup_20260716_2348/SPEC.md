...
Masala Arrange Helper is a four-part vocal arranging workbench inside MuseScore Studio. It lets an arranger choose a key and mode, build a chord progression from vertical chord cards, decide exactly which singer has each chord member, choose each singer's octave, audition the voicing, and write the result onto four mapped score staves.
...
A card can be clicked to append it or dragged to a location in the progression lane. A Custom button adds a freely editable chord.

### 4.3 Progression lane
...
Every part row also has controls for:
...
### 4.6 Score connection

The default mapping assumes this score order:
